...
if continueOrNot then
       ...
       if objfilesys.fileexists(server.mappath(intranetLocation & name)) then
            ...
            responseText = "<table border=1><tr><td>Line</td><td>lot no</td><td>item</td><td>quantity</td><td>start time</td><td>working minutes</td><td>order key</td><td>screw</td><td>vip</td><td>remark</td></tr>"
		    ...
		    'FANAR+ go-live resivion
		    txt_order_key = mid(txt_order_key, 2,len(txt_order_key) -4)
                    txt_VIP = mulItems(10)
                    ...
                    'vbCrLF
                   responseText = responseText & "<tr><td>" & curProdLine & "</td><td style = 'mso-number-format:\@;'>" & txt_lot_no & "</td><td>" & txt_item_no & "</td><td>" & planned_production_qty & "</td><td>" & startTime & "</td><td>" & workingMinutes & "</td><td>" & txt_order_key & "</td><td>" & pullScrew & "</td><td>" & txt_VIP & "</td><td>" & txt_remark & "</td></tr>"
